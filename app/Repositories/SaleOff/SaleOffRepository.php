...
class SaleOffRepository extends RepositoryAbstract implements SaleOffInterface, CrudableInterface {
     ...
     */
    public function __construct(SaleOff $saleoff) {
     ...
     */
    public function all() {
        return $this->saleoff->where('lang', $this->getLang())->get();
    }

    public function getCategory($id) {
        return $this->saleoff->where('lang', $this->getLang())->Where('id', $id)->first();
    }

    public function getCategoryAll() {
        return $this->saleoff->where('lang', $this->getLang())->where('cat_parent_id', 0)->where('status', 1)->select('id', 'name')->get();
    }

    public function getCatParent() {
        return $this->saleoff->where('cat_parent_id', 0)->select('id', 'title')->get();
    }

    public function getCatSub($cat_parent_id) {
        return $this->saleoff->where('cat_parent_id', $cat_parent_id)->select('id', 'name')->get();
    }

    public function searchCatByName($searchTitle) {
        return $this->saleoff->where('name', 'like', '%' . $searchTitle . '%')->orderBy('cat_parent_id')
                        ->paginate(10);
    }
    ...
    public function paginate($page = 1, $limit = 10, $all = false) {
        ...
        $saleoffs = $query->skip($limit * ($page - 1))->take($limit)->orderBy('created_at', 'desc')->where('lang', $this->getLang())->get();

        $result->totalItems = $this->totalCategories();
        $result->items = $saleoffs->all();
    ...
    public function lists() {
        return $this->saleoff->where('lang', $this->getLang())->lists('name', 'id');
    }
    ...
    public function find($id) {
        return $this->saleoff->findOrFail($id);
    }
    ...
    public function getNewsBySlug($slug) {
        return $this->saleoff->where('slug', $slug)->where('lang', $this->getLang())->first()->news()->paginate($this->perPage);
    }
    ...
    public function getBySlug($slug) {
        return $this->saleoff->where('slug', $slug)->first();
    }
    ...
    public function create($attributes) {
        if ($this->isValid($attributes)) {
            $this->saleoff->lang = $this->getLang();
            $this->saleoff->fill($attributes)->save();
            $this->saleoff->resluggify();
    ...
    public function update($id, $attributes) {
        $this->saleoff = $this->find($id);

        if ($this->isValid($attributes)) {
            $this->saleoff->fill($attributes);
            $this->saleoff->resluggify();
            $this->saleoff->save();
            return true;
        }
    ...
    protected function totalCategories() {
        return $this->saleoff->where('lang', $this->getLang())->count();
    }
    ...
    public function hasChildItems($id) {
        $count = $this->saleoff->where('cat_parent_id', $id)->where('lang', $this->getLang())->get()->count();
        if ($count === 0) {
            return false;
        }
    ...
    public function togglePublish($id) {
        ...
        $saleoff->status = ($saleoff->status) ? 0 : 1;
        $saleoff->save();

        return Response::json(array('result' => 'success', 'changed' => ($saleoff->status) ? 1 : 0));
    }

    public function getFirstCategory($limit) {
        return $this->saleoff->orderBy('order', 'ASC')->where('lang', $this->getLang())->take($limit)->offset(0)->get();
    }

    public function getCategoryByParentId($parent_id) {
        $saleoffs = $this->saleoff->where('cat_parent_id', $parent_id)->get();
        return $saleoffs;
    }
