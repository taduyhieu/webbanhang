...
class CategoryRepository extends RepositoryAbstract implements CategoryInterface, CrudableInterface {
    ...
    public function getCatParent() {
        return $this->category->where('cat_parent_id', 0)->select('id', 'name')->get();
    }
    ...
    public function paginate($page = 1, $limit = 10, $all = false) {
        ...
        $query = $this->category->orderBy('order', 'ASC');
    ...
    protected function totalCategories() {
        return $this->category->where('lang', $this->getLang())->orderBy('cat_parent_id', 'ASC')->count();
    }
